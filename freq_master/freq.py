...
class FreqCounter(dict):
    ...
    def get_domain(self, input_df):
        url = input_df  # ['query'].iat[0]
...
# if __name__ == "__main__":
    ...
    # fc = FreqCounter()
    # fc.load('table.freq')
    # #print(fc.probability('vix')[0])
    #
    # fc.tally_str('vix', 100000)
    # print(fc.probability('apple')[0])

@app.route('/', methods=['POST', 'GET'])
def root():
    global fc
    fc = FreqCounter()
    return 'DNS anomaly detection'
...
def measure():
    if request.method == 'POST':
        ...
        dns=fc.get_domain(input)
        #fc.save('table.freq')
        if fc.probability(dns)[0] > 5:
            return 'safe'
        else:
            return 'unsafe'
...
def weigth_update():
    try:
        ...
        fc.tally_str(dns, 100000)
        fc.save('freq_master/table.freq')
...
if __name__ == '__main__':
    ...
    fc = FreqCounter()
    fc.load('freq_master/table.freq')
